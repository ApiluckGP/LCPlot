...
server <- function(input, output, session) {
  ...
  output$allelicDiscriminationPlot <- renderPlotly({
    ...
    # Precompute labels
    fam_label <- paste(input$alleleFAM, "Homozygous")
    hex_label <- paste(input$alleleHEX, "Homozygous")
    ...
    allelic_data <- data %>%
      ...
      mutate(
        Genotype = case_when(
          FAM > 1.5 * HEX & FAM > 0.5 ~ fam_label,  # FAM Dominates
          HEX > 1.5 * FAM & HEX > 0.5 ~ hex_label,  # HEX Dominates
          abs(FAM - HEX) < 0.5 & FAM > 0.5 & HEX > 0.5 ~ "Heterozygous", # Both Signals Strong
          FAM < 0.5 & HEX < 0.5 ~ "NTC", # Minimal Signal
          TRUE ~ "Undefined" # Ambiguous
        )
      )
    
    p <- ggplot(allelic_data, aes(x = FAM, y = HEX, color = Genotype)) +
      ...
      labs(
        title = "Allelic Discrimination Plot with Genotypes",
        x = paste(input$alleleFAM, "Fluorescence Intensity"),
        y = paste(input$alleleHEX, "Fluorescence Intensity"),
        color = "Genotype"
      ) +
      theme_minimal() +
      theme(
