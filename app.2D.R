  ...
  sidebarLayout(
    sidebarPanel(
      ...
      fluidRow(
        ...
        column(6, uiOutput("dyeSelection1"))  # Dye 1 selection (right)
      ),
      fluidRow(
        column(6, uiOutput("alleleAssignment2")),  # Allele 2 assignment (left)
        column(6, uiOutput("dyeSelection2"))  # Dye 2 selection (right)
      ),
      actionButton("plot", "Generate Plot"),
      checkboxInput("cluster", "Apply K-Means Clustering (k=3)", value = TRUE)  # Clustering checkbox enabled by default
...
server <- function(input, output) {
  ...
  output$dyeSelection1 <- renderUI({
    ...
    selectInput("dye1", "Select Dye 1", choices = dyes, selected = dyes[1])
  })
  ...
  output$dyeSelection2 <- renderUI({
    ...
    selectInput("dye2", "Select Dye 2", choices = dyes, selected = dyes[2])
  })
