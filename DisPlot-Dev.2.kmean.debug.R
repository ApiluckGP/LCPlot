...
server <- function(input, output, session) {
  ...
  output$allelicDiscriminationPlot <- renderPlotly({
    ...
    # Process the data and validate
    data <- process_data()
    if (is.null(data) || nrow(data) == 0) {
      stop("Processed data is empty or invalid.")
    }
    cat("Processed data preview:\n")
    print(head(data))
    cat("Columns in processed data:\n")
    print(colnames(data))
    
    # Prepare data for allelic discrimination
    allelic_data <- tryCatch({
      data %>%
        group_by(ROW, COL) %>%
        summarise(
      ...
      stop("Error during summarisation of allelic data: ", e$message)
    })
    
    # Debugging: Ensure allelic_data is valid
    if (is.null(allelic_data) || nrow(allelic_data) == 0) {
      stop("Allelic data is empty or invalid.")
    }
    cat("Allelic data preview:\n")
    print(head(allelic_data))
    
    # Perform K-means clustering and validate
    kmeans_result <- tryCatch({
      kmeans(allelic_data[, c("FAM", "HEX")], centers = input$numClusters)
    }, error = function(e) {
      stop("Error during K-means clustering: ", e$message)
    })
    ...
    hulls <- tryCatch({
      ...
      stop("Error during convex hull calculation: ", e$message)
    })
    
    # Debugging: Ensure hulls data is valid
    if (is.null(hulls) || nrow(hulls) == 0) {
      stop("Hull data is empty or invalid.")
    }
    cat("Hull data preview:\n")
    print(head(hulls))
    ...
    # Generate the plot
    p <- ggplot(allelic_data, aes_string(x = x_axis, y = y_axis, color = "Cluster")) +
      ...
      labs(
        ...
        x = paste(x_label, "Fluorescence Intensity"),
        y = paste(y_label, "Fluorescence Intensity"),
        color = "Cluster",
        fill = "Cluster"
      ) +
      theme_minimal() +
