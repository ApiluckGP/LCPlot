...
server <- function(input, output, session) {
  ...
  output$allelicDiscriminationPlot <- renderPlotly({
    ...
    if (nrow(allelic_data) == 0) {
      stop("No valid allelic data after processing.")
    }
    ...
    hulls <- allelic_data %>%
      group_by(Cluster) %>%
      summarise(hull_indices = list(chull(FAM, HEX))) %>%
      unnest(hull_indices) %>%
      mutate(hull = row_number()) %>% # Recreate row indices
      left_join(allelic_data, by = c("Cluster"))
    
    if (nrow(hulls) == 0) {
      stop("No convex hulls were generated.")
    }
